...
Master=200;
Repeat=40;
plasmid=0*ones(Master,Repeat,PoolNumPlasmid);
...
HostOrNot=1*ones(PoolNumSpecies,PoolNumPlasmid);
    ...
    for j=1:Repeat
        ...
        if isempty(warnMsg)
            for k=1:NumPlasmid
                plasmid(i,j,k)=sum(y(end,NumSpecies+k:NumPlasmid:NumSpecies+(NumSpecies-1)*NumPlasmid+k))./sum(y(end,1:NumSpecies));
                temp=etaPool(k,:,:);
